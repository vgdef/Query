                        ...
                        IF(rk.KELAS=5, 'VVIP',
                            IF(rk.KELAS=6, 'HCU/ISOLASI',
                                IF(rk.KELAS=7, 'INTENSIF',
                                    IF(rk.KELAS=8, 'ISOLASI NICU/PICU',
                                        IF(rk.KELAS=9, 'Gawat Darurat',
                                            IF(rk.KELAS=10, 'Kamar Bersalin',
                                                IF(rk.KELAS=11, 'NICU/PICU', ""))))))))))))) KELAS,
 (COUNT(rk.ID) - COUNT(case when rkt.STATUS = 2 then 1 END) )SEMUA,
    (COUNT(rk.ID) - COUNT(case when rkt.STATUS = 3 then 1 END) )TERSEDIA,
