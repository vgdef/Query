SELECT
  @JMLTT:=(SELECT COUNT(TEMPAT_TIDUR)
        ...
        , IF(SUM(DIPINDAHKAN+HIDUP+MATIKURANG48+MATILEBIH48)=0, 0 ,ROUND(((@JMLTT * (DATEDIFF('2021-05-31','2021-05-31')+1)) - SUM(HP)) / (SUM(DIPINDAHKAN+HIDUP+MATIKURANG48+MATILEBIH48)),2)) TOI
...
FROM (
    ...
    ,SUM(IF(pd.CARA IN (6,7) AND HOUR(TIMEDIFF(pp.TANGGAL, pd.TANGGAL)) >= 48,1,0)) MATILEBIH48
       ...
       FROM 	 layanan.pasien_pulang pd, pendaftaran.kunjungan pk
							  ...
							  , (SELECT TANGGAL TGL
									  FROM master.tanggal
									 WHERE TANGGAL BETWEEN '2021-05-31' AND '2021-05-31') bts
