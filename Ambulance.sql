...
#  AND
     kp.TANGGAL BETWEEN DATE_FORMAT(NOW(), "2023-06-01") AND DATE_FORMAT(NOW(), "2023-07-01")
#  AND pk.RUANGAN LIKE '1010501%'

AND tm.STATUS = 1
 AND t.ID IN(15073, 15074, 14803, 14804, 12745,12747,12749,12748,12744,12746,12750,12751,15108,15118,15227,15225,15226)
...
SELECT
    ...
    t.sarana,
    p.created_at
FROM ambulance_2.penyewas p
LEFT JOIN ambulance_2.tarif_tindakan t ON t.id=p.layanan_tindakan
WHERE p.created_at BETWEEN DATE_FORMAT(NOW(), "2023-06-01") AND DATE_FORMAT(NOW(), "2023-07-01")
AND p.status!=0