...
INNER JOIN pendaftaran.penjamin pj ON pp.NOMOR=pj.NOPEN  AND pj.JENIS= 1
...
-- CONCAT(MONTH(tg.TANGGAL),'-',YEAR(tg.TANGGAL)) = CONCAT(MONTH(NOW())-1,'-',YEAR(NOW()))
 --  AND
 tm.TANGGAL BETWEEN DATE_FORMAT(NOW(), "2023-01-01") AND DATE_FORMAT(NOW(), "2023-23-01")
#  AND pk.RUANGAN LIKE '1010101%'
AND tm.STATUS = 1
#  AND t.ID IN(15069,15070,15071,15072,1574,15075,15076,15077,15118,15225,15226,15227)
 AND t.ID IN(15118,15225,15226,15227)
  AND rt.STATUS = 1
AND pj.JENIS=1
 -- AND rads.ID = 66
