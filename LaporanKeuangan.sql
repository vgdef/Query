...
#     t.NAMA TINDAKAN,
    ...
    #         dd.SARANA_NON_AKOMODASI diskon , pppp.NAMA KASIR
    FROM pembayaran.rincian_tagihan rt
         LEFT JOIN pembayaran.tagihan_pendaftaran ptp ON rt.TAGIHAN = ptp.TAGIHAN  AND ptp.UTAMA = 1
...
#     LEFT JOIN pembayaran.diskon dd ON dd.TAGIHAN=rt.TAGIHAN#     LEFT JOIN aplikasi.pengguna pppp ON pppp.ID=dd.OLEH
WHERE  kp.STATUS=2
AND rt.JENIS=4
-- and pp.NORM=26045 --   rt.TAGIHAN = 2211140089

  AND kp.TANGGAL BETWEEN DATE_FORMAT(NOW(), '2022-01-01') AND DATE_FORMAT(NOW(), '2023-01-01');